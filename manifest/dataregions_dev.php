...
<th> Действия</th>
    ...
    <?php 
    $result = $DB->query("SELECT region_id, SUM(ischeck) as ischecksum,
    SUM(isorder) as sumisorder, SUM(gaz) as sumgaz,
    ...
    <tr>
      ...
      echo $row["region_id"];
       ...
       ?></td>
      <td><?php echo $row["sumflag"]; ?></td>
      <td><?php echo $row["ischecksum"]; ?></td>
      <td><?php echo $row["sumtreb"]; ?></td>
      <td><?php echo $row["sumgaz"]; ?></td>
      <td></td>
      <td></td>
      <td></td>
      <td><a href="datacities.php?city=<?php echo $row["region_id"];  ?>"><button class="btn btn-sm btn-info">Перейти </button></a></td>
