...
<?php include('modals.php') ?>
            ...
            <div class="card">
					<div class="card-header header-elements-inline">
						<h5 class="card-title">Сведения по городам и районам</h5>
...
<th> Действия</th>
    ...
    <?php 
    $city = $_GET['city'] ;
    $result = $DB->query("SELECT brooms.region_id, brooms.city_id, SUM(brooms.ischeck) as bischecksum, 
    ...
    FROM brooms, boilers  WHERE brooms.city_id = '$city'
    ");
    $i = 0 ; 
    foreach ($result as $row) :
        $i++ ; 
    ...
    <tr>
      ...
      $regionid = $row["city_id"];
        $result3 = $DB->query("SELECT * FROM cityname WHERE id=$regionid");
        foreach($result3 as $row3):
        $region_id = $row3["cityname"];
        echo $region_id ;  
        endforeach ;
      ...
      <td><?php echo $row["bochecksum"]; ?></td>
      <td><?php echo $row["btrebsum"]; ?></td>
      <td><a href="dataorgs.php?id=<?php echo $row["city_id"];  ?>"><button class="btn btn-sm btn-info">Перейти </button></a></td>
